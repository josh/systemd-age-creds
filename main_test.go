...
func TestAllowedPIDs(t *testing.T) {
	socketPath := t.TempDir() + "/allowedpids.sock"
	saddr := &net.UnixAddr{Name: socketPath, Net: "unix"}
	...
	opts.Accept = false
	opts.ListenFDNames = "allowedpids.sock"
	opts.ListenFDsStart = int(f.Fd())
	opts.AllowedPIDs = []int{os.Getpid()}
	...
	val, err := readCred("foo", socketPath)
	if err != nil {
		t.Errorf("expected allowed PID to succeed, got error: %v", err)
	} else if val == "" {
		t.Errorf("expected non-empty credential, got empty string")
	}
...
func TestDeniedPIDs(t *testing.T) {
	socketPath := t.TempDir() + "/allowedpids.sock"
	saddr := &net.UnixAddr{Name: socketPath, Net: "unix"}
	...
	opts.ListenFDNames = "allowedpids.sock"
	opts.ListenFDsStart = int(f.Fd())
	opts.AllowedPIDs = []int{os.Getpid() + 1}
	...
	_, err = readCred("foo", socketPath)
	if err == nil {
		t.Errorf("expected disallowed PID to fail, but got no error")
	}
